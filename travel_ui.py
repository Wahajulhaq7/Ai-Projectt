...
def run_ui():
    ...
    if "error" in data:
        st.error(
            f"FATAL ERROR: Could not load travel data. Please ensure {data_path} exists "
            f"and contains valid JSON.\nError: {data['error']}"
        )
        return
    ...
    interest_options = [
        "beach", "adventure", "mountains", "history",
        "nightlife", "food", "art", "gardens"
    ]

    interests = st.sidebar.multiselect(
        "4. Interests",
        interest_options,
        default=["history", "food", "adventure"]
    )
    ...
    if 'best' in st.session_state and st.session_state['best']:
        ...
        st.markdown(
            f"**Optimal Destination Chosen based on {best_dest.utility_score:.4f} Utility Score.**"
        )
        st.markdown("---")
